...
public class AutoGenerator {
  ...
  private void configureBall_5_trench() {
    var autoCommandGroup = ball_5_trench();
    autoChooser.addOption("ball_5", autoCommandGroup);
...
;
  ...
  private Command ball_5_trench() {
    ...
    new TrajectoryConfig(DriveConstants.kMax_Speed_Meters_Per_Second,
        ...
        .setEndVelocity(0)
        .setReversed(false)
        .addConstraint(new CentripetalAccelerationConstraint(0.8))
        ;

        TrajectoryConfig s_config = 
    new TrajectoryConfig(2.5,    //2
                         1.5)
        .setKinematics(DriveConstants.kDrive_Kinematics)
        .addConstraint(autoVoltageConstraint)
        .setStartVelocity(0)
    ...
    var trajectory_straight = TrajectoryGenerator.generateTrajectory(new Pose2d(0, 0, new Rotation2d(0))
    , Collections.emptyList()
    , new Pose2d(3.38, 0, new Rotation2d(0))
    , s_config);

    var trajectory_straight_r = TrajectoryGenerator.generateTrajectory(new Pose2d( 3.38,0, new Rotation2d(0))
    , Collections.emptyList()
    , new Pose2d(-2,0,new Rotation2d(0)), r_config);

    var trajectory_curve = TrajectoryGenerator.generateTrajectory(new Pose2d(-2 ,0,new Rotation2d(0))
    , Collections.emptyList()
    , new Pose2d(-2.45, -1.65, new Rotation2d(-3.10)) , config);

    var trenchpickup = m_drive.createCommandForTrajectory(trajectory_straight).alongWith(new InstantCommand(m_intake::start_Intake, m_intake));
    var center_go = m_drive.createCommandForTrajectory(trajectory_straight_r);
    var center_level = m_drive.createCommandForTrajectory(trajectory_curve).alongWith(new InstantCommand(m_shooter::shoot_Start, m_shooter));

    return new InstantCommand(m_intake::open_Intake, m_intake)
    .andThen(trenchpickup)
    .andThen(m_intake::stop_Intake, m_intake)
    .andThen(center_go)
    .andThen(center_level)
    .andThen(m_indexer::start_Indexer_auto, m_indexer)
    .andThen(m_intake::start_Intake, m_intake)
    ;
